...
void IRAM_ATTR ExternalInputInterrupt()
{
  if ((hal.ReadInputRegisters() & B00010000) == 0)
  {
    ...
    // I added this 13/07/21
    SERIAL_DEBUG.println("Emergency shut down was triggered!");
    emergencyStop = true;
  }
}
...
void LoadConfiguration()
{
  ...
  //strcpy(mysettings.mqtt_server, "192.168.0.26"); // set this to local ip
  strcpy(mysettings.mqtt_server, "test.mosquitto.org");
  strcpy(mysettings.mqtt_username, "frawa"); // username:root 
  strcpy(mysettings.mqtt_password, "123456"); // password:diyBmsSun
